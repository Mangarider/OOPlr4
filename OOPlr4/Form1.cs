...
namespace OOPlr4
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            RedPen = new Pen(Color.LightBlue, 1);
        ...
        private void pictureBox1_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                if (c.getColor())
                    c.draw(e.Graphics, RedPen);
                else
                    c.draw(e.Graphics, BlackPen);
            }
        }
        ...
        private void Form1_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            foreach (Circle c in circles)
            {
                if (c.inCircle(CursorX, CursorY))
                {
                    if (model.get_check_multiple())
                        c.setColor(false);
                    else if (check) c.setColor(false);
                    else c.setColor(true);
                    model.setCtrl(false);
                    check = false;
                    c.color_changed();
                }
            ...
            if (check)
            {
                ...
                Circle circle = new Circle(CursorX, CursorY);
                circles.Add(circle);
                model.setCtrl(false);
            }
            panel1.Invalidate();
